...
def rust_table(fn, name, doc):
    out = ['', f'/// {doc}', f'pub static {name}: [Option<Row>; 256] = [']
    for op in range(256):
        r = fn(op)
        if r is None:
            out.append(f'    None, // {op:02X}')
            continue
        m, nb, tt, undoc = r
        m = m.replace('\\', '\\\\').replace('"', '\\"')
        out.append(f'    row("{m}", {nb}, "{tt}", {"false" if undoc else "true"}), // {op:02X}')
    out += ['];']
    return out
...
def rust_fixture():
    L = [FIXTURE_HEADER]
    L += rust_table(unprefixed, 'UNPREFIXED', 'Unprefixed opcodes. `CB`/`DD`/`ED`/`FD` are prefixes, not instructions.')
    L += rust_table(cb, 'CB', '`CB`-prefixed opcodes.')
    L += rust_table(ed, 'ED', '`ED`-prefixed opcodes.')
    L += rust_table(lambda o: ddfd(o, 'IX'), 'DD', '`DD`-prefixed opcodes. `FD` is identical with `IY` for `IX`.')
    L += rust_table(lambda o: ddcb(o, 'IX'), 'DDCB', '`DD CB d`-prefixed opcodes. `FD CB d` is identical with `IY` for `IX`.')
    L.append('')
    return L
...
if __name__ == '__main__':
    ...
    if what == 'grids':
        ...
        print('\n'.join(L))
    elif what == 'rust-fixture':
        print('\n'.join(rust_fixture()))
    else:
        sys.exit(f'unknown mode {what!r}: expected grids, lists or rust-fixture')
