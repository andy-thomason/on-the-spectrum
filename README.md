...
cargo run --release --features ui --bin zxbevy -- game.z80   # ...from a snapshot
cargo run --bin zxheadless                   # boot the ROM and print the screen
cargo run --bin zxheadless -- --load game.z80 --frames 200     # a snapshot, headless
cargo run --bin zxheadless -- --frames 250 --wav boot.wav       # ...and record its sound
...
| `tests/beeper.rs` | The beeper: the fractional sample rate not drifting, a held level decaying instead of sitting as a DC offset, and a supersonic square wave coming out quiet rather than aliased |
